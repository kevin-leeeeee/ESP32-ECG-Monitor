...
5. **行動電源 (Power Bank - 乾淨直流電源)**：⚠️ **極為關鍵的硬體推薦！**
   - **為什麼重要**：AD8232 是高靈敏度模擬前端，極易受到電源噪聲干擾。
   - **避免工頻干擾**：若使用電腦 USB 或家用 AC-DC 充電器直接為 ESP32 供電，家用電網的 50Hz/60Hz 工頻電磁噪訊會透過電源線嚴重污染 ECG 訊號，導致波形充滿雜訊、無法辨識 R 波。
   - **乾淨直流電 (DC)**：使用獨立行動電源供電，能提供最純淨的直流電，徹底隔離電網噪聲，獲得清晰、平滑的心電波形。

### 2. AD8232 與 ESP32 接線對照表
...
### 4. 異常心跳間隔濾除與校正 (Artifact Correction)

量測過程中若受試者晃動、手震或電極滑動，容易導致 R 波偵測漏判（產生大於 1500ms 的假間期）或噪聲多判（產生小於 300ms 的假間期），使 HRV 指標（SDNN, RMSSD）嚴重偏離真實值。為此，演算法在計算 HRV 前實作了兩階段人為干擾濾波器：

- **中位數偏差濾波 (Median RR Filter)**：計算整段 RR 間期的中位數（`medianRR`），自動剔除偏離中位數超過 **±25%** 的極端離群值。這有效過濾了大部分因為漏判或雜訊多判產生的假心跳。
- **相鄰變異濾波 (Adjacent RR Filter)**：生理狀態下，前後相鄰的兩次心跳時間差很少發生劇烈突變。如果當前心跳間隔與前一次相比變化大於 **20%**，則判定為身體晃動產生的假性變異點，予以剔除，確保 SDNN 與 RMSSD 的統計準確性。

---

## ⚙️ 安裝與執行指引

本專案由多個部分組成，包含 ESP32 韌體端、React Native 手機 App 與 PC 接收端。
詳細的安裝步驟、環境配置、依賴庫安裝、軟體執行指南以及藍牙權限設定，請參閱：
👉 **[INSTALL.md (系統安裝與執行指南)](INSTALL.md)**
